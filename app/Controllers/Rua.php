...
class Rua extends ResourceController
{
    ...
    public function index()
    {
        ...
        if (isset($data) && (count($data) > 1)) {
            foreach ($data as $value) {
                $data = [
                    'cod' => newGuid(),
                    'bairro_cod' => isset($value->bairro_cod) ? $value->bairro_cod : null,
            ...
            $data = [
                'cod' => newGuid(),
                'bairro_cod' => isset($data->bairro_cod) ? $data->bairro_cod : null,
    ...
    {
        $data = $this->model->where('cod', $cod)->first();
        return $this->respond($data);
    }
}
